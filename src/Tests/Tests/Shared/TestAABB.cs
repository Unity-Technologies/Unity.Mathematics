...
namespace Unity.Mathematics.Tests
{
    ...
    public class TestAABB
    {
        ...
        {
            var aabb = new AABB { Center = new float3(1.0f, -1.0f, 1.0f), HalfExtents = new float3(0.5f) };
        ...
        public static void Size_trivial()
        {
            ...
            TestUtils.AreEqual(float3.zero, aabb.Extents);
        }
        ...
        public static void Size()
        {
            ...
            TestUtils.AreEqual(new float3(1.0f), aabb.Extents);
        }
        ...
        public static void Contains_float3_nonzero_extents()
        {
            ...
            AABB aabb = new AABB { HalfExtents = extents };
        ...
        {
            var aabb_outer = new AABB { HalfExtents = new float3(0.5f, 0.5f, 0.5f) };
        ...
        {
            var aabb1 = new AABB { HalfExtents = new float3(0.5f, 0.5f, 0.5f) };
            var aabb2 = new AABB { Center = new float3(0.5f, 0.5f, 0.5f), HalfExtents = new float3(0.5f)};
        ...
        public static void DistanceSq_point_inside()
        {
            ...
            var aabb = new AABB { Center = center, HalfExtents = new float3(0.5f) };
        ...
        public static void Transform_trivial()
        {
            ...
            TestUtils.AreEqual(float3.zero, transformedAabb.HalfExtents);
        }

        [TestCompiler]
        public static void Transform_translation()
        {
            var localAabb = new AABB { Center = float3.zero, HalfExtents = new float3(1.0f) };
            var transform = float4x4.Translate(new float3(1.0f));
            var transformedAabb = AABB.Transform(transform, localAabb);

            TestUtils.AreEqual(new float3(1.0f), transformedAabb.Center);
            TestUtils.AreEqual(localAabb.HalfExtents, transformedAabb.HalfExtents);
        }
        ...
        public static void Transform_rotation()
        {
            ...
            TestUtils.AreEqual(float3.zero, transformedAabb.Center);
            TestUtils.AreEqual(new float3( 1.5f, 1.70710678118f, 1.5f), transformedAabb.HalfExtents, 0.0001f);
        }
