        ...
        unsafe public static void quaternion_to_euler()
        {
            ...
            for (int order = 0; order < 6; ++order)
            {
                for (int i = 0; i < testAngleCount; ++i)
                {
                    ...
                    quaternion expected = TestRefEulerToQuat(testAngles[i], RotationOrder.Default);

                    float3 actualEulers = quaternion.Euler(expected, (RotationOrder)order);
                    quaternion actualQuat = TestRefEulerToQuat(actualEulers, (RotationOrder)order);
